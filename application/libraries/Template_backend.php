...
class CI_Template_backend {
   ...
   function CI_Template_backend()
   {
      // Copy an instance of CI so we can use the entire framework.
      $this->CI =& get_instance();
      
      // Load the template config file and setup our master template and regions
      include(APPPATH.'config/template_backend'.EXT);
      if (isset($template))
      {
         $this->config = $template;
         $this->set_template_backend($template['active_template']);
      }
   }
   ...
   function set_template_backend($group)
   {
   ...
   function set_master_template_backend($filename)
   {
   ...
   function add_template_backend($group, $template, $activate = FALSE)
   {
      if ( ! isset($this->config[$group]))
      {
   ...
   function initialize($props)
   {
      ...
      $this->parse_template_backend = isset($props['parse_template']) ? $props['parse_template'] : FALSE;
   }
   ...
   function render($region = NULL, $buffer = FALSE, $parse = FALSE)
   {
      ...
      { 
         ...
         { //echo $this->master.'-'; print_r ($this->output); echo '_'.$this->buffer;
